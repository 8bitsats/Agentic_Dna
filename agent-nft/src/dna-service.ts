...
export class DNAService {
   ...
   */
  async generateDNA(options: DNAGenerationOptions = {}): Promise<DNAResponse> {
    const {
      numTokens = 8,
      temperature = 0.8,
      topK = 1,
    ...
    try {
      ...
      while (retries < this.maxRetries) {
        try {
          console.log(`Calling NVIDIA DNA API with sequence: ${startSequence}, tokens: ${numTokens}`);
   ...
   */
  private generateMockDNA(options: DNAGenerationOptions = {}): DNAResponse {
    const {
      numTokens = 8,
      startSequence = 'ACTGACTGACTGACTG'
    } = options;

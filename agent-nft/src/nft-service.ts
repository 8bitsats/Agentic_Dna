...
import { DNAService, DNAResponse } from './dna-service';
...
export class NFTService {
  ...
  private async mintSolanaNFT(metadata: NFTMetadata, imagePath: string, options: MintOptions): Promise<NFTMintResult> {
    if (options.useDnaService && this.dnaService && !metadata.agentDNA) {
      const dnaResult = await this.dnaService.generateDNA({ 
      ...
      // The dnaResult from DNAService doesn't have visualizationUrl property
      // If needed, we would need to create it using a DNAVisualizer
      // For now, leave it undefined until visualization is implemented

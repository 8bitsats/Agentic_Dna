...
export class MetaplexCoreService {
  ...
  public initializeWithPrivateKey(privateKey: string) {
    ...
    this.payer = {
      publicKey: this.umi.keySigner.fromSecretKey(secretKey).publicKey,
      secretKey: secretKey,
      signMessage: (message: Uint8Array) => ({
        signature: this.umi.signMessage(message, secretKey),
        publicKey: this.umi.keySigner.fromSecretKey(secretKey).publicKey,
      }),
      signTransaction: (transaction: any) => this.umi.signTransaction(transaction, secretKey),
    };
